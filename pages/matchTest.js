...
    // --- ส่วนที่ 1: ตรวจสอบผู้เล่นที่ไม่ว่าง (Busy) ---
...
    // --- ส่วนที่ 2: กรองและเรียงลำดับผู้เล่นที่ว่าง (Available) ---
    // MODIFIED: กรองผู้เล่นที่ไม่อยู่ในแมตช์, ไม่ได้จ่ายเงินออกไปแล้ว และไม่ได้ระบุว่าไม่อยู่
    const availableMembers = members.filter(mem => 
      !busyPlayers.has(mem.name) && 
      !earlyPayers.includes(mem.name) && 
      !mem.isAbsent // เพิ่มเงื่อนไขการตรวจสอบสถานะ "ไม่มา"
    ); 
...
    // MODIFIED: เปลี่ยนจากการสุ่มแบบเต็มรูปแบบเป็นการเรียงลำดับตามจำนวนเกมที่เล่นน้อยที่สุดก่อน
    // สมมติว่าใน object ของผู้เล่นมี property ชื่อ 'gamesPlayed' ที่เก็บจำนวนเกมที่เล่นแล้ว
    const sortedByGamesPlayed = [...availableMembers].sort((a, b) => a.gamesPlayed - b.gamesPlayed);
    
    // MODIFIED: เลือก 4 คนแรกจากรายชื่อที่เรียงแล้ว
    const selectedPlayers = sortedByGamesPlayed.slice(0, 4);
...
        if (isBrowser && loggedInEmail) {
  ...
  const handleEndGroup = async () => {
    ...
    if (!isExcelSaved) {
      Swal.fire("กรุณาบันทึกข้อมูลก่อน", "คุณต้องบันทึกไฟล์ Excel ก่อนจึงจะสามารถปิดก๊วนได้", "warning");
      return;
    }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          Swal.fire("บันทึกสำเร็จ!", "บันทึก Match เข้าประวัติสำเร็จ", "success");
          await resetSession();
          fetchMembers();
        } catch (error) {
...
const exportMatchDataToExcel = () => {
    if (matches.length === 0) {
        Swal.fire("ยังไม่มีข้อมูลแมตช์", "กรุณาเพิ่มข้อมูลแมตช์ก่อนบันทึก Excel", "warning");
        return;
    }

    const summaryMap = {};
    const matchDate = matches[0]?.date || new Date().toLocaleDateString('th-TH');
    const today = new Date().toLocaleDateString('th-TH');

    const matchDetails = matches.map((match, index) => {
        const players = [match.A1, match.A2, match.B1, match.B2].filter(Boolean);
        const balls = parseInt(match.balls) || 0;
        const result = match.result;
        const matchId = match.matchId || `Match-${index + 1}`;

        players.forEach((p) => {
            if (!summaryMap[p]) {
                summaryMap[p] = { name: p, gamesPlayed: 0, wins: 0, ballsUsed: 0 };
            }
            summaryMap[p].gamesPlayed += 1;
            summaryMap[p].ballsUsed += balls;
        });

        if (result === "A") {
            [match.A1, match.A2].forEach((p) => {
                if (p) summaryMap[p].wins += 1;
            });
        } else if (result === "B") {
            [match.B1, match.B2].forEach((p) => {
                if (p) summaryMap[p].wins += 1;
            });
        }
        ...
        return {
            ...
            "B2": match.B2 || "",
            "จำนวนลูก": balls,
            "ผลการแข่งขัน": result === "A" ? "ทีม A ชนะ" : result === "B" ? "ทีม B ชนะ" : result === "DRAW" ? "เสมอ" : "",
        };
    });

    const playerSummary = Object.values(summaryMap).sort((a, b) => a.name.localeCompare(b.name)).map((m) => ({
        "ชื่อผู้เล่น": m.name,
        "เกมที่เล่น": m.gamesPlayed,
        "ชนะ": m.wins,
        "ลูกที่ใช้": m.ballsUsed,
    }));
    
    // สร้าง workbook และ worksheet
    const wb = XLSX.utils.book_new();

    // **ส่วนที่แก้ไข: สร้าง Worksheet โดยกำหนดตำแหน่งของแต่ละส่วนอย่างชัดเจน**
    const wsSummary = {};
    XLSX.utils.sheet_add_aoa(wsSummary, [[`วันที่จัดแมตช์: ${matchDate}`]], { origin: "A1" });
    XLSX.utils.sheet_add_aoa(wsSummary, [[`วันที่บันทึก: ${today}`]], { origin: "A2" });
    const summaryHeaders = Object.keys(playerSummary[0] || {});
    XLSX.utils.sheet_add_aoa(wsSummary, [summaryHeaders], { origin: "A4" });
    XLSX.utils.sheet_add_json(wsSummary, playerSummary, { origin: "A5", skipHeader: true });

    const wsDetails = {};
    XLSX.utils.sheet_add_aoa(wsDetails, [[`วันที่จัดแมตช์: ${matchDate}`]], { origin: "A1" });
    XLSX.utils.sheet_add_aoa(wsDetails, [[`วันที่บันทึก: ${today}`]], { origin: "A2" });
    const detailsHeaders = Object.keys(matchDetails[0] || {});
    XLSX.utils.sheet_add_aoa(wsDetails, [detailsHeaders], { origin: "A4" });
    XLSX.utils.sheet_add_json(wsDetails, matchDetails, { origin: "A5", skipHeader: true });

    // จัดกึ่งกลางในทุกเซลล์ของทั้ง 2 sheet
    const centerAllCells = (ws) => {
    ...
    XLSX.utils.book_append_sheet(wb, wsDetails, "รายละเอียดแมตช์");

    const dateParts = matchDate.split(/\/|-/);
    const fileName = `${dateParts[0]}-${dateParts[1]}-${dateParts[2]}-badminton.xlsx`;
    
    const wbout = XLSX.write(wb, { bookType: "xlsx", type: "array" });
    saveAs(new Blob([wbout], { type: "application/octet-stream" }), fileName);
    setIsExcelSaved(true);
    Swal.fire("บันทึกสำเร็จ", "ข้อมูลถูกบันทึกเป็นไฟล์ Excel แล้ว", "success");
};
      ...
      <div className="card control-panel-card">
        <div className="control-panel">
          ...
          <div className="action-time-group">
            {isOpen && (
              <button
                onClick={exportMatchDataToExcel}
                className="action-button"
                style={{ backgroundColor: "#4b4e4b",}}
              >
                บันทึก Excel
              </button>
            )}
