...
const getMvpIcon = (rank) => {
  switch (rank) {
    ...
    case 3:
      return "🥉"; // Bronze Medal
    default:
      return "🥉"; // Generic Medal for 4th onwards
  }
};
...
const MatchDetails = () => {
  ...
  const handleAddCoupon = () => {
    ...
    if (memberAlreadyHasCoupon) {
      Swal.fire("Duplicate Coupon", `${currentMemberSelection} ได้รับการใช้คูปองแล้วในเซสชันนี้`, "warning");
      return;
    }
    ...
    if (couponIdAlreadyApplied) {
        Swal.fire("คูปองถูกใช้แล้ว", `คูปอง ${currentCouponSelection.reason} ได้รับการใช้ไปแล้วกับสมาชิกคนอื่นในเซสชันนี้`, "warning");
        return;
    }
    ...
    Toast.fire({ icon: 'success', title: `คูปอง ${currentCouponSelection.reason} ถูกเพิ่มให้ ${currentMemberSelection}` });
  };
  ...
  const handleRemoveAppliedCoupon = async (indexToRemove) => {
    ...
    try {
        ...
        Swal.fire({ title: "การคำนวณถูกอัปเดตแล้ว", text: "ข้อมูลค่าใช้จ่ายถูกคำนวณใหม่โดยอัตโนมัติ กรุณาตรวจสอบและกด 'บันทึกประวัติการชำระ' อีกครั้ง", icon: "info", confirmButtonText: "รับทราบ" });
    } catch (err) {
  ...
  const handleCalculateClick = () => {
    ...
    if (!isAnyCourtFeeFilled) { Swal.fire("Invalid Input", "กรุณากรอกค่าสนาม, ค่าสนาม/เกม, หรือค่าสนาม (ระบุต่อคน) อย่างใดอย่างหนึ่ง และต้องไม่เป็นค่าติดลบ", "warning"); return; }
    if (ballPrice === "" || parseFloat(ballPrice) < 0) { Swal.fire("Invalid Input", "กรุณากรอกราคาลูกละ และต้องไม่เป็นค่าติดลบ", "warning"); return; }
    if (organizeFee === "" || parseFloat(organizeFee) < 0) { Swal.fire("Invalid Input", "กรุณากรอกค่าจัดก๊วน และต้องไม่เป็นค่าติดลบ", "warning"); return; }
    ...
    Swal.fire({ icon : "success", title: "คำนวณค่าใช้จ่ายสำเร็จ", text: "โปรดตรวจสอบการคำนวณในตาราง" });
  };
  ...
  const handleSaveToRanking = async () => {
    ...
    try {
      ...
      Swal.fire({ icon: "success", title:"บันทึกสถิติ Ranking สำเร็จ", text: `บันทึกสถิติ เดือนที่ ${monthYearId} สำเร็จ กรุณาอย่าบันทึกซ้ำ!!` });
  ...
  const handleSavePaymentHistory = async () => {
    if (Object.keys(memberCalculations).length === 0) { Swal.fire("Insufficient Data", "กรุณาคำนวณค่าใช้จ่ายก่อนบันทึกประวัติการชำระ", "warning"); return; }
    if (!matchData || !matchData.matchDate || !matchId) { Swal.fire("Incomplete Data", "ไม่พบข้อมูล Match ID หรือวันที่ Match สำหรับบันทึกประวัติการชำระ", "error"); return; }
    try {
      const usersRef = collection(db, "users");
      const userQuery = query(usersRef, where("email", "==", loggedInEmail));
      const userSnap = await getDocs(userQuery);
  ...
  const handleExportToExcel = () => {
    ...
    const ws_data = [["No.", "ชื่อ", "จำนวนเกม", "จำนวนลูก", "ราคารวมลูกที่ใช้", "ค่าสนาม (เฉลี่ย)", "ค่าจัดก๊วน", "คูปองส่วนลด", "จำนวนชนะ", "จำนวนเสมอ", "คะแนน", "Total (บาท)", "จ่ายแล้ว"]];
  ...
  const handleDownloadImage = async () => {
    if (!tableRef.current || Object.keys(memberCalculations).length === 0) { Swal.fire("ไม่มีข้อมูล", "กรุณาคำนวณข้อมูลก่อนดาวน์โหลดรูปภาพ", "warning"); return; }
    try {
  ...
  const handleDownloadGameDetailsImage = async () => {
      if (!gameDetailsTableRef.current || !matchData?.matches?.length) { Swal.fire("ไม่มีข้อมูล", "ไม่มีข้อมูลเกมสำหรับดาวน์โหลดรูปภาพ", "warning"); return; }
      try {
  ...
  return (
    <div style={{ padding: "30px", backgroundColor: "#f7f7f7", minHeight: "100vh", fontFamily: "'Kanit', sans-serif" }}>
      ...
      <div style={{ marginBottom: "25px", padding: "20px", border: "1px solid #e0e0e0", borderRadius: "8px", backgroundColor: "#fff" }}>
        ...
        <div style={{ marginBottom: "20px", padding: "15px", border: "1px solid #d0d0d0", borderRadius: "5px", backgroundColor: "#f9f9f9" }}>
          ...
          <div style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
            <div><label style={{ display: "block", marginBottom: "5px", fontSize: "14px", color: "#333" }}>ค่าสนาม (รวม):</label><input type="number" value={courtFee} onChange={handleCourtFeeChange} placeholder="ค่าสนามรวม" style={{ padding: "8px 12px", borderRadius: "5px", border: "1px solid #ccc", fontSize: "15px", width: "120px" }}/></div>
            <div><label style={{ display: "block", marginBottom: "5px", fontSize: "14px", color: "#333" }}>ค่าสนาม/เกม:</label><input type="number" value={courtFeePerGame} onChange={handleCourtFeePerGameChange} placeholder="ค่าสนาม/เกม" style={{ padding: "8px 12px", borderRadius: "5px", border: "1px solid #ccc", fontSize: "15px", width: "120px" }}/></div>
            <div><label style={{ display: "block", marginBottom: "5px", fontSize: "14px", color: "#333" }}>ค่าสนาม (ระบุต่อคน):</label><input type="number" value={fixedCourtFeePerPerson} onChange={handleFixedCourtFeePerPersonChange} placeholder="ค่าสนามต่อคน" style={{ padding: "8px 12px", borderRadius: "5px", border: "1px solid #ccc", fontSize: "15px", width: "120px" }}/></div>
          </div>
        </div>
        <div style={{ marginBottom: "20px", padding: "15px", border: "1px solid #d0d0d0", borderRadius: "5px", backgroundColor: "#f9f9f9" }}>
          <h4 style={{ fontSize: "16px", marginBottom: "10px", color: "#333" }}>ค่าลูกและค่าจัดก๊วน:</h4>
          <div style={{ display: "flex", gap: "20px", flexWrap: "wrap", marginBottom: "15px" }}>
            <div><label style={{ display: "block", marginBottom: "5px", fontSize: "14px", color: "#333" }}>ราคาลูกละ:</label><input type="number" value={ballPrice} onChange={(e) => setBallPrice(e.target.value)} placeholder="ราคาลูกละ" style={{ padding: "8px 12px", borderRadius: "5px", border: "1px solid #ccc", fontSize: "15px", width: "120px" }} /></div>
            <div><label style={{ display: "block", marginBottom: "5px", fontSize: "14px", color: "#333" }}>ค่าจัดก๊วน:</label><input type="number" value={organizeFee} onChange={(e) => setOrganizeFee(e.target.value)} placeholder="ค่าจัดก๊วน" style={{ padding: "8px 12px", borderRadius: "5px", border: "1px solid #ccc", fontSize: "15px", width: "120px" }} /></div>
          </div>
            ...
            <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", alignItems: "flex-end" }}>
              <div>
                ...
                <select value={currentCouponSelection.id} onChange={(e) => { const couponId = e.target.value; const selected = unappliedAvailableCoupons.find(coupon => coupon.id === couponId); setCurrentCouponSelection(selected ? { id: selected.id, amount: selected.amount, reason: selected.reason } : { id: "", amount: 0, reason: "" }); }} style={{ padding: "8px 12px", borderRadius: "5px", border: "1px solid #ccc", fontSize: "15px", width: "180px" }}>
                  ...
                  {unappliedAvailableCoupons.map(coupon => (<option key={coupon.id} value={coupon.id}>{coupon.reason} ({coupon.amount} บาท)</option>))}
                </select>
              </div>
              <div>
              ...
              <button onClick={handleAddCoupon} style={{ backgroundColor: "#17a2b8", color: "#fff", padding: "8px 15px", borderRadius: "5px", border: "none", cursor: "pointer", fontSize: "14px" }}>เพิ่มคูปอง</button>
            </div>
            {appliedCoupons.length > 0 && (
              <div style={{ marginTop: "15px", borderTop: "1px solid #eee", paddingTop: "10px" }}>
                <h5 style={{ fontSize: "15px", marginBottom: "8px", color: "#555" }}>คูปองที่ใช้ในเซสชันนี้:</h5>
                <ul style={{ listStyle: "none", padding: 0 }}>
                  {appliedCoupons.map((item, index) => (<li key={index} style={{ background: "#e9ecef", padding: "8px", borderRadius: "4px", marginBottom: "5px", display: "flex", justifyContent: "space-between", alignItems: "center" }}><span>{item.reason} ({item.amount} บาท) สำหรับ {item.memberName}</span><button onClick={() => handleRemoveAppliedCoupon(index)} style={{ background: "#dc3545", color: "white", border: "none", borderRadius: "3px", padding: "5px 8px", cursor: "pointer", fontSize: "12px" }}>X</button></li>))}
                </ul>
              </div>
            )}
          </div>
      ...
      {/* ADDED: MVP Count Selector */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
        <div>
          {isDataCalculated && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <label htmlFor="mvp-select" style={{ fontSize: '14px', fontWeight: 'bold' }}>กำหนดจำนวน MVP:</label>
              <select 
                id="mvp-select"
                value={mvpCount}
      ...
      <div ref={tableRef} style={{ overflowX: "auto", border: "1px solid #e0e0e0", borderRadius: "8px", backgroundColor: "#fff", padding: '12px' }}>
        {showDateInDownloadImage && (<div style={{ padding: "8px 0", marginBottom: "10px", fontSize: "16px", fontWeight: "bold", textAlign: "left", color: "#333", borderBottom: "1px solid #eee" }}>สรุปค่าใช้จ่าย Match วันที่: {formatDate(matchData.matchDate)}</div>)}
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px" }}>
          <thead>
            <tr style={{ backgroundColor: "#323943", color: "white" }}>
              ...
              <th style={{ padding: "12px 10px", borderRight: "1px solid #444", textAlign: "center" }}>จำนวนเสมอ</th>
          ...
          <tbody>
            {Object.keys(memberCalculations).length === 0 ? (
              <tr><td colSpan={"13"} style={{ textAlign: "center", padding: "20px", color: "#777" }}>{isDataCalculated ? "ไม่มีข้อมูลการคำนวณ" : "กรุณากรอกค่าใช้จ่ายแล้วกด 'คำนวณค่าใช้จ่าย' เพื่อดูรายละเอียด"}</td></tr>
            ) : (
        ...
        <div ref={gameDetailsTableRef} style={{ overflowX: "auto", border: "1px solid #e0e0e0", borderRadius: "8px", backgroundColor: "#fff", padding: '12px' }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px" }}>
            ...
            {matchData.matches && matchData.matches.length > 0 && (
              <tfoot>
                <tr style={{ backgroundColor: "#f0f0f0", fontWeight: "bold" }}>
                  <td colSpan={5} style={{ padding: "10px", textAlign: "right", borderRight: "1px solid #eee" }}>รวมลูกที่ใช้ทั้งหมด:</td>
